...
void OLED_Show_TimeNum(uint8_t x, uint8_t y, uint8_t fontSize, char data, uint8_t mode)
{
    uint8_t font_dat;
    uint8_t i;
...
//    for (i=0; i<zh_num*2; i+=2) //一个个查找字库
//    {
//        //需同时判断汉字的低字节和高字节
...
//    ch1 = i/2;
    ...
    for (uint8_t i=0; i<fontSize/8; i++)    // 多少行
    {
        for (uint8_t j=0; j<fontSize/2; j++)                      // 多少列
        {
            font_dat = zh_font[(data-'0')*fontSize*fontSize/2/8 + i*fontSize/2+j];
...
void OLED_Show_ZH(uint8_t x, uint8_t y, uint8_t fontSize, char ch1, char ch2, uint8_t mode)
{
    ...
    for (uint8_t i=0; i<fontSize/8+(fontSize%8!=0?1:0); i++)    // 多少行
    {
        for (uint8_t j=0; j<fontSize; j++)                      // 多少列
        {
            font_dat = zh_font[ch1*zh_hex_num+i*fontSize+j];
#if 1
...
void OLED_Show_EN(uint8_t x, uint8_t y, uint8_t fontSize, char ch, uint8_t mode)
{
    uint8_t font_dat;

    for (uint8_t i=0; i<fontSize/8+(fontSize%8!=0?1:0); i++)    // 行数
    {
        for (uint8_t j=0; j<fontSize/2; j++)                    // 列数
        {
            switch (fontSize)
            {
                case 14:
                    font_dat = EN_FONT14[(ch-' ')*EN_FONT14_HEX_NUM+i*fontSize/2+j];
                    break;
                case 16:
                    font_dat = EN_FONT16[(ch-' ')*EN_FONT16_HEX_NUM+i*fontSize/2+j];
                    break;
            }
#if 1
...
//            OLED_Show_Char(x, y, *str, 12, mode);
            OLED_Show_EN(x, y, fontSize, *str, mode);
            x += fontSize/2;
            str++;
        }
        else
        {
            OLED_Show_ZH(x, y, fontSize, *str, *(str+1), mode);
            x += fontSize;
            str += 2;
        }
    }
